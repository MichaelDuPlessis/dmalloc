...
static inline bool is_chunk_fully_coalesced(Chunk *chunk) {
    ...
    size_t total_block_space = (uintptr_t)chunk + chunk->mmap_allocation.size - (uintptr_t)first_block_ptr;

    // Check if there is exactly one block and it occupies all available space.
    return chunk->block_head != NULL &&
           chunk->block_head->next == NULL &&
...
start_search:;
    ...
    while (current_chunk) {
        ...
        while (current_block) {
            ...
            if (current_block->size >= total_size) {
                ...
                // A remaining fragment only needs to be large enough for a Block struct.
    ...
    // Instead, we loop again. The new chunk is at the head and will be found instantly.
    goto start_search;
}
...
void free_list_free(void *ptr, Chunk *chunk) {
    ...
    // if current is right after the current header than the blocks can be coalesced
