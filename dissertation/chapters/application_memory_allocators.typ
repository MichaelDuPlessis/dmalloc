...
The basic idea behind pool allocators is that a chunk of memory (also known as a buffer) is split into smaller chunks. The allocator then keeps track of which chunks are available and when a block of memory is requested a free chunk is provided. This means that some metadata needs to be stored to make it clear which blocks of memory are free and which are not. A common approach to this is by making use of a free list which is just a linked list @cormen2022introduction of the available chunks of memory. The head of the linked list points to the next available chunk of memory. Due to the data associated with each allocation it is possible for the pool allocator to deallocate individual objects. When an object is deallocated it is just made the new head of the free list @gay1998memory @boostpool @whyusepool.
