...
This chapter will cover common memory allocation algorithms, strategies and types of memory allocators as well as discuss some designs of existing memory allocator implementations.
...
The basic idea behind pool allocators is that a chunk of memory (also known as a buffer) is split into smaller chunks. The allocator then keeps track of which chunks are available and when a block of memory is requested a free chunk is provided. This means that some metadata needs to be stored to make it clear which blocks of memory are free and which are not. A common approach to this is by making use of a free list which is commonly implemented as a linked list @cormen2022introduction of the available chunks of memory. The head of the linked list points to the next available chunk of memory. Due to the data associated with each allocation it is possible for the pool allocator to deallocate individual objects. When an object is deallocated it is just made the new head of the free list @gay1998memory @boostpool @whyusepool.
...
As seen in @fig:poolallocator_empty the pool allocator appears just like a normal linked list.
...
) <freelist_allocator_linkedlist_before_deallocation>
...
As can be seen from @fig:buddy_allocator the memory is recursively split until it can be no longer. This assists with minimising fragmentation by trying to use as little memory as possible. The allocator may make blocks too small that a new piece of memory cannot be allocated when this happens blocks of memory are coalesced with one another until a block of memory large enough to fit the requested allocation is created.

Deallocation works by marking the block of memory as free and adding it to the free list. This means that freeing memory is fast as it is $O(1)$. This allocator also minimises internal fragmentation by trying to decrease the size of blocks and also decrease external fragmentation as memory blocks next to one another can be made bigger if necessary.

The downsides of this allocator is that while deallaction is fast, allocation my not necessarily be since there may be any number of recursive calls involved as well as an unknown amount of memory blocks may need to be coalesced. This means it is difficult to determine exactly how long an allocation will take as a unknown number of events may need to occur.
...
@fig:binallocator depicts a typical bin allocator with 3 bins. The green blocks is unallocated memory, the red blocks allocated memory and the orange blocks wasted memory due to fragmentation.

The bin memory allocator tries to reduce fragmentation in an efficient way, while the buddy allocator may still perform better in terms of decreased fragmentation it has the downside of needing to use recursion which increases allocation and deallocation times.

The downsides of a bin allocator are that while allocation is fast since finding a bin is fast there is a high likelihood of internal fragmentation. If we have bins of 4 bytes, 8 bytes and 32 bytes than if many allocations of 16 bytes were requested they would be allocated into the 32 byte bin but this means for every 16 byte allocation 16 bytes are wasted. Due to this bin allocators number of bins and bin sizes need to be thought about carefully to reduce the severity of internal fragmentation.
...
This chapter explained common memory allocation strategies and analysed real world exampls of memory allocators. Attention should be paid to the bin memory
allocation strategy as it is used in the design of the memory allocator proposed in this mini-dissertation.
