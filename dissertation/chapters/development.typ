...
3. The bin allocator allocates all its memory.
...
2. The bin allocator tries to allocate 1 object (1).
  ...
  3. The $(5)$th page is given the bin allocator.
...
The green blocks in the @example_page_cache represent memory pages that that the page cache has cached while the red represent an empty cell. @fig:example_page_cache has the steps with the corresponding actions that are taken.
...
2. The previous free block is continuous to the memory to be freed and the blocks are coalesced into a bigger block.
3. The next free block is continuous to the memory to be freed and the blocks are coalesced into a bigger block
4. Both the previous and the next free block is continuous to the memory to be freed and the blocks are coalesced into a bigger block
If after a free the free list is empty (it has no allocated memory) the page is returned to the page store. Determining if the free list is free is done by checking if the head block of the free list's size is all the available memory in the free list.
As seen in @fig:sub_allocator_freelist free blocks are connected through a linked list with the head of the list stored in the metadata of the free list.
...
The page allocator is used for any memory allocated that is larger than a page size. It is the simplest allocator and is used for very large objects or for preallocating large amounts of memory. It follows the same structure as the previous two allocators where the first item in its memory page is the kind of allocation followed by metadata of the allocation. The page allocator directly allocates memory pages using the `mmap` system call.

=== Implementation

It will allocates the amount of pages necessary for the requested amount of memory. There is exactly one memory allocation for each huge allocation. This means that if two allocations of sufficient size are requested at least two memory pages are used (depending on the size of allocation) per allocation. This means that allocation is very fast since no region of memory needs to be searched but it is slow however in the sense since a system call needs to be made each time an allocation occurs. At the same time freeing memory is also fast since no searching has to occur but does also require a system call. Freeing memory is done by extracting the header information and than calling `unmmap` to release the memory back to the operating system.
#figure(
  caption: [How the free lists are connected.],
...
Using pointer arithmatic over array indexing since there is not repeated arithmetic to derive the value's address this can lead to significant speedups as there a pointer addition is a single operation whereas array indexing
...
In the scenario in @lst:branch most of the time the index provided for the bit to be marked will be a valid index so it is expected to evaluate to false.
...
The implementation of the allocator will only function on UNIX based operating systems as it requires the `mmap` system call and it currently does not support multithreaded environments.
